...
for row, m in enumerate(sal_ses):
    ...
    axes[row].plot(tvec, sniff_av[:,m+nmice,0], label = 'FAM DREADD', color = 'C7', linestyle = '--')
    axes[row].fill_between(tvec, sniff_av[:,m+nmice,0] + sniff_sem[:,m+nmice,0], sniff_av[:,m+nmice,0] - sniff_sem[:,m+nmice,0], alpha = 0.2, color = 'C7')
...
for row, m in enumerate(sal_ses):
    ...
    axes[row].errorbar(pvec_f, sniff_1bin_av[:,m+nmice,0], sniff_1bin_sem[:,m+nmice,0], label = 'FAM DREADD', color = 'C7', linestyle = '--')

    axes[row].errorbar(pvec_n, sniff_1bin_av[:10,m,1], sniff_1bin_sem[:10,m,1], label = 'NOV SAL', color = 'C0', linestyle = '-')
    axes[row].errorbar(pvec_n, sniff_1bin_av[:10,m+nmice,1], sniff_1bin_sem[:10,m+nmice,1], label = 'NOV DREADD', color = 'C0', linestyle = '--')
        
    axes[row].set_ylabel(u"\u0394" + ' sniffing [inh/sec]')
    
    ax2 = axes[row].twinx()
    ax2.set_yticks([])
    mouse_id = sniffs[row]['unique_id'][7:12]
    ax2.set_ylabel('Mouse ' + mouse_id)
    
    
axes[row].legend()
axes[row].set_xticklabels(['PB1', 'PB2', 'PB4', '2', '4', '6', '8', '10'])
axes[row].set_xlabel('Presentation number')
...
for row, m in enumerate(sal_ses):
    ...
    axes[row].plot(tvec, pup_av[:,m+nmice,0], label = 'FAM DREADD', color = 'C7', linestyle = '--')
    axes[row].fill_between(tvec, pup_av[:,m+nmice,0] + pup_sem[:,m+nmice,0], pup_av[:,m+nmice,0] - pup_sem[:,m+nmice,0], alpha = 0.2, color = 'C7')
...
for row, m in enumerate(sal_ses):
    ...
    axes[row].errorbar(pvec_f, pup_1bin_av[:,m+nmice,0], pup_1bin_sem[:,m+nmice,0], label = 'FAM DREADD', color = 'C7', linestyle = '--')

    axes[row].errorbar(pvec_n, pup_1bin_av[:10,m,1], pup_1bin_sem[:10,m,1], label = 'NOV SAL', color = 'C0', linestyle = '-')
    axes[row].errorbar(pvec_n, pup_1bin_av[:10,m+nmice,1], pup_1bin_sem[:10,m+nmice,1], label = 'NOV DREADD', color = 'C0', linestyle = '--')
